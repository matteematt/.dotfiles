...
# `CLICOLOR_FORCE=1` is needed to force color output in `ls`, along with `-G` for mac and `--color=always` for linux
...
fzfLsPreview() {
	echo "fzf --ansi --tac --header \"$1\" --preview 'CLICOLOR_FORCE=1 ls -al -G --color=always $2{}'"
}
